...
class PyInstArchive:
    ...
    MAGIC = b"MEI\014\013\012\013\016"  # Magic number which identifies pyinstaller 这是八进制格式
    ...
    def open(self): # 首先获取文件指针
        try:
            self.fPtr = open(self.filePath, "rb")               # 读取exe文件，字节形式——多用seek定位比较安全？以前都是直接read
            self.fileSize = os.stat(self.filePath).st_size      # 获取exe文件大小
        except:
    ...
    def checkFile(self):    # 接着检查文件格式
        ...
        while True:
            ...
            data = self.fPtr.read(chunkSize)        # 从后往前读取，每次读取8192字节

            offs = data.rfind(self.MAGIC)           # 找魔数，因为魔数在后头
        ...
        if b"python" in self.fPtr.read(64).lower():     # 确认pyinstaller版本，看后面64字节种是否有python
    ...
    def getCArchiveInfo(self):
        try:
            if self.pyinstVer == 20:
                ...
                )   # 大端读取，首先是八字节字符、无符号4字节、无符号四字节、有符号四字节、有符号四字节、64字节字符
                # 分别对应魔数、、python3x.dll
        except:
        ...
        )   # 计算python版本 比如python3.9 pyver就是309
    ...
    def parseTOC(self):
        ...
        while parsedLen < self.tableOfContentsSize:
            ...
            (
                entryPos,               # 压缩数据开始位置
                cmprsdDataSize,         # 压缩数据大小
                uncmprsdDataSize,       # 解压数据大小
                cmprsFlag,              # 是否压缩
                typeCmprsData,          # 压缩类型
                name,                   # 文件名称
            ) = struct.unpack(
            ...
            parsedLen += entrySize
        print("[+] Found {0} files in CArchive".format(len(self.tocList)))  # 相当于一个解压信息收集
    ...
    def extractFiles(self, one_dir):        # 开始提取文件
        ...
        for entry in self.tocList:
            ...
            if entry.cmprsFlag == 1:
                data = zlib.decompress(data)        # 开始解压
            ...
            if entry.typeCmprsData == b"s":
                ...
                if data[2:4] == b"\r\n":    # 判断pyinstaller版本
                    ...
                    if entry.name.endswith("_crypto_key"):  # 检查是否加密
                        print(
                            "[+] Detected _crypto_key file, saving key for automatic decryption"
                        )
                ...
                if entry.typeCmprsData == b"z" or entry.typeCmprsData == b"Z":
                    self._extractPyz(entry.name, one_dir)   # 这个地方相比pyinstxtractor有变化
    ...
    def _fixBarePycs(self):
        for pycFile in self.barePycList:
            with open(pycFile, "r+b") as pycFile:
                ...
                pycFile.write(self.pycMagic)    # 重写文件头
    ...
    def _writePyc(self, filename, data):    # 写入pyc文件，这里补充了以前没有的魔数头
    ...
    def _extractPyz(self, name, one_dir):
        ...
        with open(name, "rb") as f:
            ...
            try:
                toc = load_code(f, pycHeader2Magic(pyzPycMagic))    # 就是这里了用了xdis.unmarshal的load_code
            except:
                print(
